...
target="${2}"
if [[ ! "${target}" ]]; then
  target="./tmp/${source##*/}" # leave only last component of path
...
#   width="$(echo ${resolution} | grep -oE '^[[:digit:]]+')"
  ...
  cmd+=" ${static_params} -vf scale=w=${width}:h=${height}:force_original_aspect_ratio=decrease,pad=width=ceil(iw/2)*2:height=ceil(ih/2)*2"
